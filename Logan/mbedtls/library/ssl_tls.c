...
/* Length of the "epoch" field in the du header */
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_1) || defined(MBEDTLS_SSL_PROTO_TLS1_2)
        /*
         * Prepend per-du IV for block cipher in TLS v1.1 and up as per
...
int mbedtls_ssl_fetch_input( mbedtls_ssl_context *ssl, size_t nb_want )
{
    ...
    if( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM )
    {
         ...
         * It could be the rest of the current du (while fetching the
         * header) and/or some other records in the same datagram.
         */

        /*
         * Move to the next du in the already read datagram if applicable
         */
        if( ssl->next_record_offset != 0 )
        {
            ...
            if( ssl->in_left != 0 )
            {
                MBEDTLS_SSL_DEBUG_MSG( 2, ( "next du in same datagram, offset: %d",
        ...
        /*
         * A du can't be split accross datagrams. If we need to read but
         * are not at the beginning of a new du, the caller did something
         * wrong.
         */
        if( ssl->in_left != 0 )
        {
...
int mbedtls_ssl_write_record( mbedtls_ssl_context *ssl )
{
    ...
    MBEDTLS_SSL_DEBUG_MSG( 2, ( "=> write du" ) );
    ...
    if( !done )
    {
        ...
        MBEDTLS_SSL_DEBUG_MSG( 3, ( "output du: msgtype = %d, "
                            "version = [%d:%d], msglen = %d",
                       ssl->out_hdr[0], ssl->out_hdr[1], ssl->out_hdr[2],
                     ( ssl->out_len[0] << 8 ) | ssl->out_len[1] ) );

        MBEDTLS_SSL_DEBUG_BUF( 4, "output du sent to network",
...
static int ssl_reassemble_dtls_handshake( mbedtls_ssl_context *ssl )
{
     ...
     * If yes, finalize it, else ask to read the next du.
     */
    ...
    {
        /*
         * We'got more handshake messages in the same du.
         ...
         */
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "last fragment not alone in its du" ) );
        return( MBEDTLS_ERR_SSL_FEATURE_UNAVAILABLE );
    }
    ...
    {
        /*
         * We've got more data in the buffer after the current du,
 ...
 * Bit n is set iff du number in_window_top - n has been seen.
 *
 * Usually, in_window_top is the last du number seen and the lsb of
 * in_window is set. The only exception is the initial state (du number 0
 * not seen yet).
 */
#if defined(MBEDTLS_SSL_DTLS_ANTI_REPLAY)
static void ssl_dtls_replay_reset( mbedtls_ssl_context *ssl )
...
/*
 * Update replay window on new validated du
 */
                           ...
                           unsigned char *obuf, size_t buf_len, size_t *olen )
{
    ...
    /*
     * Structure of ClientHello with du and handshake headers,
 ...
 *
 * Called by ssl_parse_record_header() in case we receive an epoch 0 du
 ...
 *
 * mbedtls_ssl_read_record() will ignore the du if anything else than
 ...
 * uint16 length;
 *
 * Return 0 if header looks sane (and, for DTLS, the du is expected)
 * MBEDTLS_ERR_SSL_INVALID_RECORD if the header looks bad,
 * MBEDTLS_ERR_SSL_UNEXPECTED_RECORD (DTLS only) if sane but unexpected.
 *
 * With DTLS, mbedtls_ssl_read_record() will:
 * 1. proceed with the du if this function returns 0
 * 2. drop only the current du if this function returns UNEXPECTED_RECORD
 * 3. return CLIENT_RECONNECT if this function return that value
 * 4. drop the whole datagram if this function returns anything else.
 * Point 2 is needed when the peer is resending, and we have already received
 * the first du from a datagram but are still waiting for the others.
 */
static int ssl_parse_record_header( mbedtls_ssl_context *ssl )
{
    int major_ver, minor_ver;

    MBEDTLS_SSL_DEBUG_BUF( 4, "input du header", ssl->in_hdr, mbedtls_ssl_hdr_len( ssl ) );

    ssl->in_msgtype =  ssl->in_hdr[0];
    ssl->in_msglen = ( ssl->in_len[0] << 8 ) | ssl->in_len[1];
    mbedtls_ssl_read_version( &major_ver, &minor_ver, ssl->conf->transport, ssl->in_hdr + 1 );

    MBEDTLS_SSL_DEBUG_MSG( 3, ( "input du: msgtype = %d, "
                        "version = [%d:%d], msglen = %d",
                        ssl->in_msgtype,
    ...
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "unknown du type" ) );
     ...
     * silently dropping the du (but not the whole datagram), and we only
    ...
    if( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM )
    {
        ...
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "du from another epoch: "
             ...
             * access the first byte of du content (handshake type), as we
             * have an active transform (possibly iv_len != 0), so use the
             * fact that the du header len is 13 instead.
             */
            if( ssl->conf->endpoint == MBEDTLS_SSL_IS_SERVER &&
                ssl->state == MBEDTLS_SSL_HANDSHAKE_OVER &&
                rec_epoch == 0 &&
        ...
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "replayed du" ) );
            return( MBEDTLS_ERR_SSL_UNEXPECTED_RECORD );
        }
#endif
    }
...
/*
 * If applicable, decrypt (and decompress) du content
 */
static int ssl_prepare_record_content( mbedtls_ssl_context *ssl )
{
    int ret, done = 0;

    MBEDTLS_SSL_DEBUG_BUF( 4, "input du from network",
 ...
 * RFC 6347 4.1.2.7) and continue reading until a valid du is found.
 *
 */
int mbedtls_ssl_read_record( mbedtls_ssl_context *ssl )
{
    ...
    MBEDTLS_SSL_DEBUG_MSG( 2, ( "=> read du" ) );
...
int mbedtls_ssl_read_record_layer( mbedtls_ssl_context *ssl )
{
     ...
     *
     * (4) Application data:
     *     Don't do anything - the du layer provides
    ...
    if( ssl->in_hslen != 0 )
    {
        ...
        /*
         * Get next Handshake message in the current du
         */
        ...
        if( ssl->in_hslen < ssl->in_msglen )
        {
            ...
            MBEDTLS_SSL_DEBUG_BUF( 4, "remaining content in du",
                                   ssl->in_msg, ssl->in_msglen );
        }
        else
        {
     ...
     * Step B
     *
     * Fetch and decode new du if current one is fully consumed.
     *
     */

    if( ssl->in_msglen > 0 )
    {
        /* There's something left to be processed in the current du. */
        return( 0 );
    }

    /* Need to fetch a new du */

#if defined(MBEDTLS_SSL_PROTO_DTLS)
read_record_header:
#endif

    /* Current du either fully processed or to be discarded. */
    ...
    if( ( ret = ssl_parse_record_header( ssl ) ) != 0 )
    {
            ...
            ret != MBEDTLS_ERR_SSL_CLIENT_RECONNECT )
        {
            ...
            {
                /* Skip unexpected du (but not whole datagram) */
                ssl->next_record_offset = ssl->in_msglen
                                        + mbedtls_ssl_hdr_len( ssl );

                MBEDTLS_SSL_DEBUG_MSG( 1, ( "discarding unexpected du "
                                            "(header)" ) );
            }
            else
            {
                /* Skip invalid du and the rest of the datagram */
                ssl->next_record_offset = 0;
                ssl->in_left = 0;

                MBEDTLS_SSL_DEBUG_MSG( 1, ( "discarding invalid du "
                                            "(header)" ) );
            }
    ...
    /* Done reading this du, get ready for the next one */
#if defined(MBEDTLS_SSL_PROTO_DTLS)
    if( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM )
    ...
    if( ( ret = ssl_prepare_record_content( ssl ) ) != 0 )
    {
        ...
        if( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM )
        {
                ...
                ret == MBEDTLS_ERR_SSL_INVALID_MAC )
            {
                ...
                MBEDTLS_SSL_DEBUG_MSG( 1, ( "discarding invalid du (mac)" ) );
                goto read_record_header;
            }
     ...
     *
     * If the du we received is not a handshake message, however, it
...
/*
 * Check du counters and renegotiate if they're above the limit.
 */
static int ssl_check_ctr_renegotiate( mbedtls_ssl_context *ssl )
{
    ...
    MBEDTLS_SSL_DEBUG_MSG( 1, ( "du counter limit reached: renegotiate" ) );
...
int mbedtls_ssl_read( mbedtls_ssl_context *ssl, unsigned char *buf, size_t len )
{
    ...
    if( ssl->in_offt == NULL )
    {
        ...
        if( ssl->in_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE )
        {
             ...
             * - Fail (TLS) or silently drop du (DTLS) in other cases.
             */
